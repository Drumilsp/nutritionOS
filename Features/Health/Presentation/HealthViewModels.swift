...
enum HealthState: Equatable { case loading, connected, disconnected, syncing, permissionDenied, error(String) }
...
final class HealthSettingsViewModel: ObservableObject {
    ...
    init(requestPermissionsUseCase: RequestHealthPermissionsUseCase, syncHealthDataUseCase: SyncHealthDataUseCase, disconnectHealthUseCase: DisconnectHealthUseCase) { self.requestPermissionsUseCase = requestPermissionsUseCase; self.syncHealthDataUseCase = syncHealthDataUseCase; self.disconnectHealthUseCase = disconnectHealthUseCase }
