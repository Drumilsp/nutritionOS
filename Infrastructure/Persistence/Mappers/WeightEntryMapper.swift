...
enum WeightEntryMapper {
    static func toDomain(_ entity: WeightEntryEntity) -> WeightEntry {
        WeightEntry(id: entity.id, weight: entity.weight, recordedAt: entity.recordedAt)
    }

    static func toEntity(_ entry: WeightEntry) -> WeightEntryEntity {
        WeightEntryEntity(id: entry.id, weight: entry.weight, recordedAt: entry.recordedAt)
    }
}
